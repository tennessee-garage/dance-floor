...
class FlashBang(Base):
    ...
    def set_burst_intensity(self):
        ...
        self.burst_intensity = self.value_in_range(
            self.BURST_INTENSITY_MIN, self.BURST_INTENSITY_MAX
        )
    ...
    def set_sparkle_trigger(self):
        self.sparkle_trigger = (
            (self.burst_red + self.burst_green + self.burst_blue) / 3
    ...
    def add_sparkles(self, pixels):
        for x in range(0, 64):
            ...
            self.sparkles[x] += 1
            self.burst_blue *= 0.9995
            self.burst_red *= 0.9995
            self.burst_green *= 0.9995
