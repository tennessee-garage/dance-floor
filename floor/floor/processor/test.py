...
class Test(Base):
    ...
    def __init__(self, **kwargs):
        ...
        self.cycles = []
        # full-floor cycles
        self.cycles.append({"floor": self.init_floor(red), "duration": self.SLOW_CYCLE})
        self.cycles.append({"floor": self.init_floor(blue), "duration": self.SLOW_CYCLE})
        self.cycles.append({"floor": self.init_floor(green), "duration": self.SLOW_CYCLE})
        ...
        for c in ["red", "blue", "green"]:
            for i in range(0, (self.FLOOR_WIDTH * self.FLOOR_HEIGHT)):
                ...
                floor[i] = eval(c)
                self.cycles.append({"floor": floor, "duration": self.FAST_CYCLE})
