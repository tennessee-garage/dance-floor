...
class Controller:
    ...
    def set_bpm(self, bpm, downbeat=None):
        logger.info(f"Setting bpm to: {bpm}")
        self.bpm = float(bpm)
    ...
    def set_brightness(self, factor):
        ...
        logger.info(f"Set brightness to: {int(self.brightness * 100)}%")

    def handle_input_event(self, event_name, num, value):
        logger.debug(f"input event: {event_name}: {num} -> {value}")
        if event_name == "playlist_ranged_value":
            ...
            logger.warning(f"Ignoring unknown event {event_name}")
    ...
    def delay(self):
        ...
        if elapsed < self.frame_seconds:
            ...
            logger.debug(f"Over by {elapsed - self.frame_seconds}")
